...
for(i in 1:ncol(df)){
breaks <- pretty(range(df[[colnames(df)[i]]]), n = nclass.FD(df[[colnames(df)[i]]]), min.n = 1)
bwidth <- breaks[2]-breaks[1]
gg0[[i]] <- ggplot(df, aes(df[[colnames(df)[i]]])) + geom_histogram(fill=I("white"),colour="black",alpha=I(.3),binwidth=bwidth, aes(y = ..density..)) + 
  geom_density(aes(colour = "Densidade")) + 
  stat_function(fun = dnorm, args = list(mean = mean(df[[colnames(df)[i]]]), sd = sd(df[[colnames(df)[i]]])), aes(colour = "Dist.Normal"),size = 0.5) + 
...
for (i in 1:length(gg0)) {
  ...
  normal = ifelse(pvalue>0.05,'Possui distribuição normal','Não possui distribuição normal' )
...
for(i in 1:ncol(df)){
gg0[[i]] <- ggplot(df, aes(y = df[[colnames(df)[i]]])) + geom_boxplot()
}
